...
class Balle2Encoder(nn.Module):
    ...
    def build_encoder(self):
        return torch.nn.Sequential(
            nn.Conv2d(2, self.channel, 3, stride=2, padding=1, padding_mode='zeros'),
            self.norm(self.channel),
            nn.Conv2d(self.channel, self.channel, 3, stride=1, padding=1, padding_mode='zeros'),
            self.norm(self.channel),
            nn.Conv2d(self.channel, self.channel, 3, stride=1, padding=1, padding_mode='zeros'),
            self.norm(self.channel),
            nn.Conv2d(self.channel, self.last_channel, 3, stride=1, padding=1, padding_mode='zeros')
        )
...
class Balle2Decoder(nn.Module):
    ...
    def build_decoder(self):
        return torch.nn.Sequential(
            Upsample(self.last_channel, self.channel, 3, stride=1, padding=1, output_padding=0),
            self.norm(self.channel, inverse=True),
            Upsample(self.channel, self.channel, 3, stride=1, padding=1, output_padding=0),
            self.norm(self.channel, inverse=True),
            Upsample(self.channel, self.channel, 3, stride=1, padding=1, output_padding=0),
            self.norm(self.channel, inverse=True),
            Upsample(self.channel, 2, 3, stride=2, padding=1, output_padding=1)
        )
