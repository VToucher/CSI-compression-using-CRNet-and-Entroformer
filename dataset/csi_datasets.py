...
class csiDatasets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # csi_norm = (csi_reshape - csi_reshape.min()) / (csi_reshape.max() - csi_reshape.min())
        csi_norm = F.softmax(csi_reshape, dim=0)  # 按第0维softmax
        # csi_norm = (csi_reshape - csi_reshape.mean()) / csi_reshape.std() + 0.5
        return csi_norm
