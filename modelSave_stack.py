...
class ModelSave_stack:
    def __init__(self, config, stack_size):
        self.config = config
        # self.record_txt = open(self.config.workdir + "/result_stack_%s_%02d.csv"%(config.trainset, config.cr), "w")
        self.record_txt = open(self.config.workdir + "/result_stack_softmax.csv", "w")
    ...
    def model_save(self, model, save_path, loss):
        ...
        if saveIndex != None:
            ...
            # savePath_new = path + ("_checkpoint_%s_%02d.model" % (self.config.trainset, self.config.cr))
            savePath_new = path + ("_checkpoint_softmax.model")
